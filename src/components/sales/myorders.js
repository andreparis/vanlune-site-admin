...
import { getOrdersByFilter,
    updateOrders } from '../../services/orders';
...
export class MyOrders extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            games: [],
            categories: [],
            filter: {game:1,attendedById:user['id']},
            isLoding: false,
            isLoadingGames: true,
    ...
    handlerEditRows = async (row, action) => {
        let status = 2;
        if (action == "Cancel")
            status = 3;
        this.state.isLoding = true;
        ...
        for(var i=0; i < row.length; i++) {
            let order = {
                id : String(row[i]).replace(action,""),
                status
            };
            orders.push(order);
        }
                                        ...
                                        {this.state.isLoding ?
                                            ...
                                            this.state.order && this.state.order.length > 0 ?
                                                <Datatable
                                                    ...
                                                    multiSelectColNames={["Finish","Cancel"]}
